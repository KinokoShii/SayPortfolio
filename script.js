function updateTime() {
    ...
    const time = parts.filter(part => part.type !== 'timeZoneName').map(part => part.value).join('');
...
window.addEventListener("scroll", () => {
  ...
  if (video.classList.contains('centered') && !snapped) {
    const widthPx = Math.round(initialWidth + (targetWidthPx - initialWidth) * progress);
  ...
  video.style.left = '50%';
  const currentProgress = Math.min(Math.max(1 - rect.top / windowHeight, 0), 1);
  const widthPx = Math.round(initialWidth + (targetWidthPx - initialWidth) * currentProgress);
  video.style.width = `${widthPx}px`;
  video.style.transform = 'translateX(-50%)';
  // force reflow then re-enable transitions after a short delay so layout settles
  void video.offsetWidth;
...
window.addEventListener("scroll", () => {
  ...
  if (under) {
    ...
    underProgress = Math.min(Math.max((windowHeight - underRect.top) / windowHeight, 0), 1);
